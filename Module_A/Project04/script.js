...
function calculate() {
    // Get currency codes for base and target currencies
    const baseCode = baseCurrency.value;
    const targetCode = targetCurrency.value;
    // Execute Fetch API
    fetch(`https://v6.exchangerate-api.com/v6/3d1b99f14f6922f98bf0bde1/latest/${baseCode}`)
        .then(res => res.json() )
        .then(data => {
...
// Listen for click on the flip button
